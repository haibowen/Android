...
public final class LightController extends EventBus<Integer> implements LightPeripheral.Callback {
    ...
    private static final int TAG_NOTIFY_DATA = 201;
    private static final int TAG_NOTIFY_ENABLE = 202;
    ...
    private static final int TAG_OTA_CHECK = 502;
    ...
    protected boolean resetDeviceAddress() {
        ...
        enableNotifyCmd.tag = TAG_RESET_MESH_ADDRESS_NOTIFY_DATA;

        this.sendCommand(this.notifyCallback, enableNotifyCmd);
        ...
        this.sendCommand(this.normalCallback, opcode, 0x0000, params, TAG_RESET_MESH_ADDRESS);
        byte[] params1 = new byte[]{(byte) 0xFF, (byte) 0xFF};
        this.sendCommand(this.normalCallback, opcode, 0x0000, params1, TAG_NORMAL_COMMAND);
    ...
    public void enableNotification() {
        ...
        Command writeNotifyCmd = Command.newInstance();
        writeNotifyCmd.type = Command.CommandType.WRITE;
        writeNotifyCmd.data = data;
        writeNotifyCmd.serviceUUID = serviceUUID;
        writeNotifyCmd.characteristicUUID = characteristicUUID;
        writeNotifyCmd.tag = TAG_NOTIFY_ENABLE;

        Command enableNotifyCmd = Command.newInstance();
        enableNotifyCmd.type = Command.CommandType.ENABLE_NOTIFY;
        enableNotifyCmd.serviceUUID = serviceUUID;
        enableNotifyCmd.characteristicUUID = characteristicUUID;
        enableNotifyCmd.tag = TAG_NOTIFY_DATA;

        this.sendCommand(this.notifyCallback, writeNotifyCmd);
        this.sendCommand(this.notifyCallback, enableNotifyCmd);
    }
    ...
    private boolean validateOta() {
        ...
        TelinkLog.d("ota send byte length : " + sendTotal);
        if (sendTotal > 0 && sendTotal % sectionSize == 0) {
            TelinkLog.d("send ota read packet " + otaPacketParser.getNextPacketIndex());
            Manufacture manufacture = Manufacture.getDefault();
            UUID serviceUUID = manufacture.getUUID(Manufacture.UUIDType.SERVICE);
            UUID characteristicUUID = manufacture.getUUID(Manufacture.UUIDType.OTA);
    ...
    private boolean sendCommand(Command.Callback callback, byte[] commandData, Object tag) {
    ...
    public boolean sendCommand(Command.Callback callback, byte opcode, int address, byte[] params, Object tag) {
        ...
        return this.sendCommand(callback, command, tag);
    }

    public boolean sendCommand(byte opcode, int address, byte[] params) {
        return this.sendCommand(opcode, address, params, TAG_NORMAL_COMMAND);
    }

    public boolean sendCommand(byte opcode, int address, byte[] params, Object tag) {
        return this.sendCommand(this.normalCallback, opcode, address, params, tag);
    }
    ...
    private final class ConnectionRunnable implements Runnable {
        ...
        public void run() {
            ...
            synchronized (LightController.this) {
                if (!LightController.this.light.isConnected()) {
    ...
    private final class OtaCommandCallback implements Command.Callback {
        ...
        public void success(Peripheral peripheral, Command command, Object obj) {
            if (command.tag.equals(TAG_OTA_CHECK)) {
                ...
                if (delay <= 0) {
                    ...
                    mDelayHandler.postDelayed(otaTask, Manufacture.getDefault().getOtaDelay());
                }
                setOtaProgressChanged();
        ...
        public void error(Peripheral peripheral, Command command, String errorMsg) {
            ...
            if (!command.tag.equals(TAG_OTA_READ)) {
                resetOta();
