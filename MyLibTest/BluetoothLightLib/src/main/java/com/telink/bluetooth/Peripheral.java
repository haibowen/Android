...
public class Peripheral extends BluetoothGattCallback {
    ...
    public final boolean requestConnectionPriority(int connectionPriority) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            return this.gatt.requestConnectionPriority(connectionPriority);
        }
        return false;
    }
    ...
    synchronized private void processCommand(CommandContext commandContext) {
        ...
        if (null != command.data)
            System.out.println("--data.len" + command.data.length + DataHelper.byteArrayToHexString(command.data));
        switch (commandType) {
            case READ:
    ...
    private void postCommandTimeoutTask() {
        ...
        this.mDelayHandler.removeCallbacks(this.mCommandTimeoutRunnable);
        this.mDelayHandler.removeCallbacksAndMessages(null);
        this.mDelayHandler.postDelayed(this.mCommandTimeoutRunnable, this.commandTimeoutMill);
    }
        ...
        if (service != null) {
            ...
            if (characteristic != null) {
                ...
                if (!this.gatt.writeCharacteristic(characteristic)) {
                    ...
                    System.out.println("-----------------Test  write characteristic error");
                }
                System.out.println("-----------------Test  write characteristic sucessful");
